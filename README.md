...
- **3D 导演台**：正式 CC0 Humanoid 角色、真实米制坐标、FK/IK 调姿、35 组基础导演姿势、Transform Gizmo、人物/摄影机/灯光关键帧、焦段/机位预设、曝光与阴影。导演视图中可直接点击人物并用 W 移动、E 旋转、R 缩放。
- **主体专属属性 Inspector**：3D 中点击人物、灯光、摄影机或摄影机机位后，右侧立即切换到该主体的独立属性页；人物显示场面调度与调姿，灯光只调当前灯，摄影机只调当前摄影参数。点击空白或“镜头级设置”返回全局镜头设置。
- **导演辅助线工具栏**：导演视图提供网格、世界轴、摄影机视锥、摄影轴、人物高度线、180°人物轴线、人物视线、运动轨迹八类结构辅助；每项可独立开关，并提供“常用 / 全部隐藏”。摄影机视锥可隐藏但不隐藏摄影机实体，也不会进入最终镜头画面。
- **摄影助手**：基于 filmback、焦距、光圈与对焦距离实时计算水平/垂直视场角、近远景深与超焦距；可一键对焦镜头目标、选中人物眼位或超焦距，帮助导演在预演阶段直接判断焦段与景深关系。
- **动作库手位优化**：叉腰、指向、防御、持弓准备/拉弓满弦/释放、双手持物、推拉、直拳、怀抱等“手必须落到具体位置”的动作使用按人物身高缩放的手部 IK；目标限制在实际肩—腕可达范围，手腕继续保留独立 FK 朝向调节，减少不同身高角色出现手位漂移和僵硬姿态。
- **灯光与环境光优化**：中性棚拍、正午、阴天、黄昏、月夜、窗光、恐怖、审讯等预设重新平衡主/补/轮廓关系，降低环境光洗平面部的问题。WebGL 预演中的环境光改为天空/地面分离的 HemisphereLight，并优化 Spot/Directional/Point 阴影分辨率、bias 与 normal bias。
...
7. 中文界面审计
8. 3D 导演台实体器材 / 主体 Inspector / 人物移动 / 摄影助手 / 动作手位 / 灯光 / 辅助线审计
9. Vitest 回归测试
10. TypeScript + Vite production build
11. Chromium 真实用户旅程与视觉审计

PDS 1.2.1 候选版回归套件为 **96 tests / 21 test files**；最终交付要求 Windows + Ubuntu + Chromium 三路均通过。
...
9. **当前选择决定右侧属性。** 选中人物、灯、摄影机或机位时，Inspector 应聚焦该对象；镜头级设置只在没有对象选择时出现，避免同时暴露不相关参数。
10. **辅助线必须可控且不污染镜头。** 网格、轴线、视线、轨迹、人物高度和摄影机视锥属于导演视图信息层，可按需显示/隐藏，不应进入最终摄影机画面。
11. **摄影参数必须给导演可读反馈。** 焦距、filmback、光圈和对焦距离不仅保存为元数据，还应转换成视场角、景深和超焦距等可验证结果。
12. **动作预设优先满足人体可达性。** 对手位等接触约束使用按人物比例缩放的 IK，手腕朝向保持独立 FK，避免只靠固定角度硬套不同角色。
13. **环境光只负责底光，不替代主光塑形。** 预设必须保留可读的主/补/轮廓关系，避免高环境光洗平人物。
14. **每次交付通过 Windows + Ubuntu + Chromium 门禁。** 不用“本机能跑”替代跨平台与真实浏览器验收。

## Roadmap

`docs/ROADMAP.md` 中定义的 **Gate 0 → Gate 5 已全部完成**。PDS 1.1/1.1.1/1.2/1.2.1 属于 1.x 产品演进，聚焦正式人物、完整中文化、导演级 FK/IK 调姿、片场器材化 3D 操作、摄影光学辅助、主体属性编辑、结构辅助线、动作人体可达性与灯光层次。后续工作属于具体 Studio 部署、第三方 DCC 实机认证、性能/UX 优化与新增生产能力。
