...
const safeName = (value: string) => {
  const clean = value.normalize('NFKD').replace(/[^A-Za-z0-9_]/g, '_').replace(/^([0-9])/, '_$1');
  return clean || 'Item';
};
...
export function shotToUsda(project: DirectorProject, shot: Shot): string {
  ...
  const lines = [
    ...
    '    metersPerUnit = 1',
    '    upAxis = "Y"',
    `    timeCodesPerSecond = ${shot.fps}`,
    `    framesPerSecond = ${shot.fps}`,
    ')',
    '',
    'def Xform "PDS" (',
    '    customData = {',
    `        string pds:projectId = ${q(project.id)}`,
    `        string pds:shotId = ${q(shot.id)}`,
    `        string pds:payloadBase64 = ${q(payload)}`,
    '        string pds:coordinateConvention = "right-handed,Y-up,-Z-forward,meter"',
    '    }',
    ')',
    '{',
    '    def Scope "Actors"',
  ...
  for (const light of shot.lights) {
    lines.push(`        def Xform "${safeName(light.id)}"`, '        {', `            custom string pds:type = ${q(light.type)}`, `            double3 xformOp:translate = ${vec(light.position)}`, '            uniform token[] xformOpOrder = ["xformOp:translate"]', `            custom float pds:intensity = ${light.intensity}`, `            custom float pds:colorTemperatureK = ${light.colorTemperatureK}`, '        }');
  }
  lines.push('    }', '}', '');
  return lines.join('\n');
}

export function projectToUsda(project: DirectorProject): string {
  const first = project.sequences[0]?.shots[0];
  if (!first) throw new Error('USD export requires at least one Shot.');
  return shotToUsda(project, first);
}
...
export function parsePdsUsda(usda: string): { projectId: string; shot: Shot } {
  ...
  const match = usda.match(/string pds:payloadBase64\s*=\s*"([^"]+)"/);
  if (!match) throw new Error('PDS USD payload metadata is missing.');
