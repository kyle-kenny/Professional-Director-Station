...
export function TopBar() {
  const mode = useDirectorStore((s) => s.mode); const setMode = useDirectorStore((s) => s.setMode); const exportProject = useDirectorStore((s) => s.exportProject); const importProject = useDirectorStore((s) => s.importProject); const undo = useDirectorStore((s) => s.undo); const redo = useDirectorStore((s) => s.redo); const canUndo = useDirectorStore((s) => s.undoStack.length > 0); const canRedo = useDirectorStore((s) => s.redoStack.length > 0); const input = useRef<HTMLInputElement>(null);
  useEffect(() => { const onKeyDown = (event: KeyboardEvent) => { const target = event.target as HTMLElement | null; const editable = target?.isContentEditable || ['INPUT', 'TEXTAREA', 'SELECT'].includes(target?.tagName ?? ''); if (editable || !(event.ctrlKey || event.metaKey)) return; const key = event.key.toLowerCase(); if (key === 'z') { event.preventDefault(); if (event.shiftKey) redo(); else undo(); } else if (key === 'y') { event.preventDefault(); redo(); } }; window.addEventListener('keydown', onKeyDown); return () => window.removeEventListener('keydown', onKeyDown); }, [redo, undo]);
