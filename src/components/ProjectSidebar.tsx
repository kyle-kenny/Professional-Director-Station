...
const cameraKeyframeId = (time: number) => `camera-keyframe:${time.toFixed(6)}`;
  ...
  return <aside className="sidebar">
    ...
    {mode === '3d' ? <>
      ...
      <div className="scene-object-tree">
        <button className={selectedObjectId === shot.camera.id ? 'active' : ''} onClick={() => selectObject(shot.camera.id)}><Camera size={14}/><span>{shot.camera.name}</span><small>摄影机 · {shot.camera.focalLengthMm.toFixed(0)} 毫米</small></button>
        {shot.camera.path.map((frame, index) => {
          const id = cameraKeyframeId(frame.time);
          return <button key={id} className={selectedObjectId === id ? 'active' : ''} onClick={() => selectObject(id)}><Camera size={13}/><span>机位 {String(index + 1).padStart(2, '0')}</span><small>{frame.time.toFixed(2)} 秒</small></button>;
        })}
      ...
      <div className="meta">也可以直接在中央 3D 视口点击人物、摄影机、机位或灯具。W 移动 · E 旋转 · Delete 删除可删除对象。</div>
    </> : <>
      <div className="project-label">镜头内容</div>
      <div className="tree"><div><Users size={15}/> 人物 <span>{shot.actors.length}</span></div><div><Camera size={15}/> 摄影机 <span>1 + {shot.camera.path.length} 机位</span></div><div><Lightbulb size={15}/> 灯光 <span>{shot.lights.length}</span></div><div><Music2 size={15}/> 音频 <span>{shot.audio.length}</span></div><div><Box size={15}/> 资产 <span>{project.assets.length}</span></div></div>
    </>}
