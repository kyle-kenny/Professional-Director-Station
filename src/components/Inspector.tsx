...
import { ageGroupZh, lightTypeZh, sexZh, shotStatusZh } from '../i18n/zhCN';
import { resolveOpenCharacter } from '../characters/characterCatalog';
...
const intensityLabel = (light: DirectorLight) => light.type === 'point' || light.type === 'spot' ? '强度（坎德拉 cd）' : light.type === 'area' ? '强度（尼特量级）' : '强度（相对值）';
...
export function Inspector() {
  ...
  return <aside className="inspector">
    <section>
      ...
      <div className="meta">时间 {playhead.toFixed(2)} 秒 · 第 {Math.round(playhead * shot.fps)} / {Math.round(shot.duration * shot.fps)} 帧<br />状态：{shotStatusZh[shot.status]} · v{shot.version} · 画幅 {shot.frameAspect.toFixed(3)}:1</div>
      {!editable && <div className="asset-message ok">已批准镜头为只读。需要修改时，请在审片工作区重新打开为新的制作中版本。</div>}
      <NumberField label="画幅比例" value={shot.frameAspect} step={0.01} disabled={!editable} onChange={setActiveShotFrameAspect} />
      <select disabled={!editable} value="" onChange={(e) => { if (e.target.value) setActiveShotFrameAspect(Number(e.target.value)); }}><option value="">选择画幅预设…</option>{frameAspectPresets.map((item) => <option key={item.label} value={item.value}>{item.label}</option>)}</select>
      <button className="wide" onClick={() => setMode('review')}>进入协作 / 审片 / 审批</button>
      <div className="meta">3D 镜头视图、2D 构图、AI 结构控制、USD 与 MP4 共用同一镜头画幅；窗口尺寸只影响显示面积，不改变构图。</div>
    </section>
    ...
    <section>
      ...
      <div className="meta">基础覆盖：男性/女性 × 儿童/青少年/成年/老年。3D 人物使用 Quaternius CC0 Humanoid 正式开源网格，演员尺寸保持真实米制比例。</div>
    </section>
    ...
    {actor && actorTransform && <section>
      ...
      {axes.map((axis) => <NumberField disabled={!editable} key={`r-${axis}`} label={`旋转 ${axis.toUpperCase()}（°）`} value={radToDeg(actorTransform.rotation[axis])} step={1} onChange={(v) => edit('rotation', axis, degToRad(v))} />)}
      {axes.map((axis) => <NumberField disabled={!editable} key={`s-${axis}`} label={`缩放 ${axis.toUpperCase()}`} value={actorTransform.scale[axis]} step={0.05} onChange={(v) => edit('scale', axis, v)} />)}
      <div className="section-title">姿势</div>
      <select disabled={!editable} value={actor.pose} onChange={(e) => applyPose(actor.id, e.target.value as PosePresetId)}>{posePresetList.map((pose) => <option key={pose.id} value={pose.id}>{pose.label} · {actionZh[pose.action] ?? pose.action}</option>)}</select>
      ...
      <div className="meta">{sexZh[actor.demographics.sex]} · {ageGroupZh[actor.demographics.ageGroup]} · {actor.demographics.ageYears} 岁 · {actor.demographics.heightM.toFixed(2)} 米<br />开源模型：{resolveOpenCharacter(actor).label} · Quaternius CC0<br />姿势：{posePresetList.find((item) => item.id === actor.pose)?.label ?? actor.pose}<br />动作：{actionZh[actor.action] ?? actor.action}<br />关键帧：{actor.path.length} · 眼高：{actor.eyeHeight} 米</div>
    </section>}
    ...
    {selectedLight && lightAtTime && <section>
      ...
      <div className="meta">类型：{lightTypeZh[selectedLight.type]} · 关键帧：{selectedLight.path.length} · {shadowSupported(lightAtTime) ? '该灯型支持实时阴影。' : '该灯型在当前 Three.js WebGL 路径不支持实时阴影。'}存在关键帧轨道时，数值/操纵器编辑会自动写入当前整帧。</div>
    </section>}
    ...
    <section>
      ...
      <select disabled={!editable} value="" onChange={(e) => { if (e.target.value) cam('focalLengthMm', Number(e.target.value)); }}><option value="">选择焦段预设…</option>{lensPresetList.map((lens) => <option key={lens.id} value={lens.focalLengthMm}>{lens.label} · {lens.use}</option>)}</select>
      <NumberField disabled={!editable} label="光圈（f 值元数据）" value={camera.aperture} step={0.1} onChange={(v) => cam('aperture', v)} />
      ...
      <div className="meta">预演视口使用真实 filmback / focal 几何；f-stop / 对焦距离作为镜头元数据保留，当前 WebGL 预演不模拟真实景深。</div>
    </section>
    ...
    <section>
      ...
      <div className="meta">ACES Filmic 预览 · 曝光补偿范围 -8EV ～ +8EV。点光源/聚光灯使用 cd；区域光使用亮度量级；平行光/环境光为预演相对强度。</div>
    </section>
  </aside>;
}
