...
export function Inspector() {
  ...
  return <aside className="inspector">
    <section>
      ...
      <div className="meta">Review/Approved、不可变版本和回滚统一由权限化 Review 工作流处理，普通 Inspector 不绕过审批状态。</div>
    </section>
    <section>
    ...
    {selectedLight && lightAtTime && <section>
      ...
      <NumberField label={intensityLabel(lightAtTime)} value={lightAtTime.intensity} step={0.1} onChange={(v) => updateLight(selectedLight.id, 'intensity', v)} />
      ...
      <label className="toggle-field"><span>Cast shadow</span><input type="checkbox" disabled={!shadowSupported(lightAtTime)} checked={shadowSupported(lightAtTime) && selectedLight.castShadow} onChange={(e) => setLightShadow(selectedLight.id, e.target.checked)} /></label>
    ...
    <section>
      ...
      <NumberField label="Aperture (f-stop metadata)" value={camera.aperture} step={0.1} onChange={(v) => cam('aperture', v)} />
