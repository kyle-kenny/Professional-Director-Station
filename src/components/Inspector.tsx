...
export function Inspector() {
  ...
  return <aside className="inspector">
    ...
    {actor && <section>
      ...
      {axes.map((axis) => <NumberField key={`p-${axis}`} label={`Position ${axis.toUpperCase()} (m)`} value={actor.transform.position[axis]} onChange={(v) => edit('position', axis, v)} />)}
      {axes.map((axis) => <NumberField key={`r-${axis}`} label={`Rotation ${axis.toUpperCase()} (°)`} value={radToDeg(actor.transform.rotation[axis])} step={1} onChange={(v) => edit('rotation', axis, degToRad(v))} />)}
      {axes.map((axis) => <NumberField key={`s-${axis}`} label={`Scale ${axis.toUpperCase()}`} value={actor.transform.scale[axis]} step={0.05} onChange={(v) => edit('scale', axis, v)} />)}
      <div className="meta">{actor.demographics.sex === 'male' ? '男' : '女'} · {actor.demographics.ageGroup} · {actor.demographics.ageYears}岁 · {actor.demographics.heightM.toFixed(2)}m<br />Pose: {actor.pose}<br />Action: {actor.action}<br />Eye: {actor.eyeHeight}m</div>
    </section>}
    <section>
      <div className="section-title">CAMERA</div>
      <NumberField label="Focal length (mm)" value={shot.camera.focalLengthMm} step={1} onChange={(v) => cam('focalLengthMm', v)} />
      <NumberField label="Aperture" value={shot.camera.aperture} step={0.1} onChange={(v) => cam('aperture', v)} />
      {axes.map((axis) => <NumberField key={`cp-${axis}`} label={`Camera ${axis.toUpperCase()} (m)`} value={shot.camera.position[axis]} onChange={(v) => camVec('position', axis, v)} />)}
    </section>
    <section>
