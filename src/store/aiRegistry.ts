...
export function approveAiOutput(outputId: string): void {
  ...
  commitProject((project) => {
    const index = project.ai.outputs.findIndex((item) => item.id === outputId);
    if (index < 0) throw new Error(`AI output ${outputId} not found.`);
    project.ai.outputs[index] = approveGeneratedMedia(project.ai.outputs[index], identity.userId, identity.role);
  });
}
...
export function rejectAiOutput(outputId: string): void {
  ...
  commitProject((project) => {
    const index = project.ai.outputs.findIndex((item) => item.id === outputId);
    if (index < 0) throw new Error(`AI output ${outputId} not found.`);
    project.ai.outputs[index] = rejectGeneratedMedia(project.ai.outputs[index], identity.userId, identity.role);
  });
}

export function promoteApprovedAiOutputToAsset(outputId: string) {
  const identity = getSessionIdentity();
  ...
  return registerProjectAsset(approvedGeneratedMediaToAsset(output, identity.userId));
}
