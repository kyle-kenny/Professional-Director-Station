...
export const cameraKeyframeSchema = z.object({ time: z.number().nonnegative(), position: vec3Schema, target: vec3Schema, focalLengthMm: z.number().min(8).max(1200).optional(), easing: easingSchema.default('ease-in-out') });
...
export const shotSchema = z.object({ id: z.string(), name: z.string(), status: z.enum(['WIP', 'REVIEW', 'APPROVED']).default('WIP'), version: z.number().int().positive().default(1), duration: z.number().positive().max(3600), fps: z.number().int().min(12).max(120), script: z.string().default(''), actors: z.array(actorSchema), camera: cameraSchema, exposureEv: z.number().min(-8).max(8).default(0), lights: z.array(lightSchema), audio: z.array(audioClipSchema), markers: z.array(timelineMarkerSchema).default([]), notes: z.array(shotNoteSchema).default([]) });
export type Shot = z.infer<typeof shotSchema>;
export const sequenceSchema = z.object({ id: z.string(), name: z.string(), shots: z.array(shotSchema) });
